#!/usr/bin/env python3
"""
CCS Direction Trace — Phi-3.5-mini-instruct
Archon, DuoNeural, 2026-05-11

KEY QUESTION: Is crystallization a Meta/Llama property or Mistral-specific?

OLMo-2-7B (Western, AllenAI): NEUTRAL (0.71x) — breaks simple Western=crystallizer
Mistral-7B-Instruct (Western): STRONG CRYSTALLIZER (9.9x)
Mistral-NeMo-12B (Western): CRYSTALLIZER (3.4x)

Phi-3.5-mini-instruct is Meta's flagship open Western-aligned model.
Different training methodology from Mistral (RLHF from Meta RLHF team, not Mistral AI).
If Llama is also a crystallizer → Western crystallization is common but not universal.
If Llama is NEUTRAL like OLMo-2 → crystallization may be Mistral-training-specific.
If Llama is a SUPPRESSOR → something very interesting is happening.

Architecture: 32 layers, hidden_dim=4096
GPU: RTX 3060 12GB — 4-bit NF4 (~4GB VRAM)
Output: /workspace/phi35_trace/direction_trace.json
"""
...
OUT_DIR  = Path("/workspace/phi35_trace")
...
print(f"[phi35] {N_CTRL} ctrl + {N_EXP} exp = {len(ALL_PAIRS)} pairs", flush=True)

print(f"\n[phi35] Loading {MODEL_ID} 4-bit NF4...", flush=True)
...
print(f"[phi35] Loaded: {n_layers} layers, hidden={d}  VRAM: {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)
...
for i, (t, f) in enumerate(ALL_PAIRS):
    if i % 5 == 0: print(f"[phi35] pair {i+1}/{len(ALL_PAIRS)}", flush=True)
...
print(f"\n[phi35] Activations saved: {act_true.shape}", flush=True)
...
print("\n[phi35] Running direction trace...", flush=True)
trace = compute_trace(act_true, act_false, N_CTRL, N_EXP)

print(f"\n{'='*65}")
print(f"Direction Trace — Phi-3.5-mini-instruct ({n_layers} layers)")
print(f"{'='*65}")
print(f"{'L':>4}  {'|r|':>8}  {'angle':>7}  {'ctrl':>6}  {'exp':>6}  {'exp_gap':>10}")
for r in trace:
    flag = " ** CRYST?" if r["layer"] > n_layers-3 and trace[r["layer"]]["magnitude"] > trace[max(0,r["layer"]-3)]["magnitude"] else ""
...
print(f"\nComparison — Western-aligned models:")
print(f"  Mistral-7B-Instruct:       9.9×  STRONG CRYSTALLIZER")
print(f"  Mistral-NeMo-12B:          3.4×  CRYSTALLIZER")
print(f"  OLMo-2-7B (AllenAI):      0.71×  NEUTRAL")
print(f"  Llama-3.1-8B (Meta):       {ratio:.2f}×  {verdict}")
...
out = {
    ...
    "comparison": {"mistral_7b": 9.9, "mistral_nemo": 3.4, "olmo2_7b": 0.71, "llama3_8b": ratio}
}
with open(OUT_DIR / "direction_trace.json", "w") as f:
    json.dump(out, f, indent=2)
print(f"\n[phi35] Results saved → {OUT_DIR}/direction_trace.json")
