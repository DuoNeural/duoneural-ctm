#!/usr/bin/env python3
"""
CCS Direction Trace — Qwen3.5-9B-Instruct
Archon, DuoNeural, 2026-05-11

TWO-PART EXPERIMENT:
  Part 1: Direction trace (does Qwen3.5 suppress like Qwen2.5?)
  Part 2: IF suppressor → Qwen-Scope SAE analysis (feature-level circuit identification)

Qwen3.5-9B is COVERED by official Qwen-Scope SAE suite.
https://huggingface.co/collections/Qwen/qwen-scope

Scientific question: Does Alibaba's newest generation (Qwen3.5 vs Qwen2.5) still suppress?
- If YES: suppression is structurally maintained across architectural generations
- If STRONGER: Alibaba is actively optimizing suppression with each release
- If WEAKER/NONE: Alibaba may have reduced suppression in response to external pressure

Also: Qwen3.5 has 262k context window (vs 128k for Qwen2.5-14B).
Does the extremely long context change the truth geometry?

Architecture: Qwen3.5-9B has 32 layers, hidden_dim=4096
GPU: RTX 3090 24GB — float16, no quantization needed
Output: /workspace/qwen35_trace/direction_trace.json
"""
...
OUT_DIR  = Path("/workspace/qwen35_trace")
...
print(f"[qwen35] {N_CTRL} ctrl + {N_EXP} exp = {len(ALL_PAIRS)} pairs", flush=True)

print(f"\n[qwen35] Loading {MODEL_ID} float16...", flush=True)
...
print(f"[qwen35] Loaded: {n_layers} layers, hidden={d}  VRAM: {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)
...
for i, (t, f) in enumerate(ALL_PAIRS):
    if i % 5 == 0: print(f"[qwen35] pair {i+1}/{len(ALL_PAIRS)}", flush=True)
...
print(f"\n[qwen35] Activations saved: {act_true.shape}", flush=True)
...
print("\n[qwen35] Running direction trace...", flush=True)
trace = compute_trace(act_true, act_false, N_CTRL, N_EXP)

print(f"\n{'='*65}")
print(f"Direction Trace — Qwen3.5-9B-Instruct ({n_layers} layers)")
print(f"{'='*65}")
...
peak_layer = max(range(len(trace)), key=lambda i: trace[i]["magnitude"])
...
print(f"Truth direction peak: L{peak_layer} (mag={trace[peak_layer]['magnitude']:.1f})")
print(f"VERDICT: {verdict}")

print(f"\n{'='*65}")
print("QWEN GENERATIONAL COMPARISON:")
print(f"  Qwen2.5-14B-Instruct (2025): 3.35× STRONG SUPPRESSOR")
print(f"  Qwen2.5-7B-Instruct  (2025): 1.28× WEAK SUPPRESSOR")
print(f"  Qwen3.5-9B-Instruct  (2026): {ratio:.2f}× {verdict}")
if ratio > 1.5:
    print("  ✓ Suppression PERSISTS across Qwen generations — structural, not incidental")
    print("  → NEXT STEP: Run Qwen-Scope SAEs to decompose truth direction into features")
    print("    https://huggingface.co/collections/Qwen/qwen-scope")
else:
    print("  ? Suppression reduced or absent in Qwen3.5 — check if architectural changes explain this")
print('='*65)
...
out = {
    ...
    "peak_magnitude": trace[peak_layer]["magnitude"],
    "qwen_scope_covered": True,
    "sae_followup_recommended": ratio > 1.5,
    "comparison": {
        "qwen25_14b": 3.35,
        "qwen25_7b": 1.28,
        "qwen35_9b": ratio,
...
print(f"\n[qwen35] Results saved → {OUT_DIR}/direction_trace.json")
print("[qwen35] If SUPPRESSOR: load Qwen-Scope SAE from HF and run feature decomposition next.")
