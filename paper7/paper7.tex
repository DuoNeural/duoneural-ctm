...
\newenvironment{genbox}[1]{%
  \begin{mdframed}[backgroundcolor=lightgray,linecolor=gray!50,linewidth=1.2pt,
    innertopmargin=6pt,innerbottommargin=6pt,innerleftmargin=8pt,innerrightmargin=8pt]
  \textbf{\small #1}\par\smallskip\small
}{%
  \end{mdframed}
}
\newenvironment{genboxgreen}[1]{%
  \begin{mdframed}[backgroundcolor=lightgray,linecolor=safecolor!80,linewidth=1.2pt,
    innertopmargin=6pt,innerbottommargin=6pt,innerleftmargin=8pt,innerrightmargin=8pt]
  \textbf{\small #1}\par\smallskip\small
}{%
  \end{mdframed}
...
Mechanistic Evidence for a Consistent RLHF Suppression Bottleneck\\
...
The bottleneck is consistent across tested architectures and appears architectural---the same 3.35$\times$ compression acts on
...
\footnote{NF4 quantization introduces low-rank perturbations to the residual stream.
The reported direction magnitudes (e.g., $|\hat{r}_{47}| = 229.9$) are measured under
NF4 conditions and may exhibit minor scaling artifacts relative to an FP16 baseline.
Transfer accuracy metrics are substantially more robust to quantization noise, as they
depend on the \emph{sign} of projections rather than absolute magnitudes.}
...
\begin{figure}[H]
\centering
\begin{genbox}{Baseline (Qwen2.5-14B, $\alpha = 0$)}
...
\end{genbox}
\vspace{4pt}
\begin{genboxgreen}{Instillation Hook ($\alpha = 2.0$)}
...
\begin{finding}
The instillation hook is provably orthogonal to general reasoning.
The intervention term is $\delta\mathbf{h} = \alpha(\mathbf{h} \cdot \hat{r}_{47})\hat{r}_{47}$.
For unrelated content---algebra, geography, or any MMLU domain---the hidden state $\mathbf{h}$
lies in a subspace geometrically orthogonal to the political truth direction $\hat{r}_{47}$,
giving $(\mathbf{h} \cdot \hat{r}_{47}) \approx 0$ and therefore $\delta\mathbf{h} \approx \mathbf{0}$.
The hook \emph{self-quenches}: it applies a near-zero perturbation to any context where the
truth direction is absent, which explains the exact $\Delta_\text{MMLU} = 0.000$ result
mathematically, not merely empirically. This distinguishes projection-based instillation
fundamentally from RepE steering vectors \citep{zou2023representation}, which apply a
constant nonzero shift to every token regardless of semantic relevance.
\end{finding}
...
A concurrent ablation on Qwen2.5-7B-Base (no RLHF, no instruction tuning) reveals that the
final-layer compression is not purely RLHF-induced: the base model shows a 1.61$\times$
compression ratio with a 50.9$^\circ$ rotation. However, the instruction-tuned counterpart
shows a different pattern---exp\_acc drops from 1.00 to 0.70, a more severe degradation than
the base model's 0.90$\to$0.80, despite a smaller raw compression ratio (1.28$\times$).
This supports a refined two-factor interpretation: (1) an \emph{architectural} bottleneck
pre-exists RLHF training, providing the structural choke point; and (2) RLHF training then
adjusts the \emph{geometric angle} at which specific truth directions enter that bottleneck,
rotating politically suppressed content toward less effective output projections while
leaving factual science truth at geometry-preserving angles. The suppression mechanism is
thus both architectural in origin and alignment-specific in targeting---RLHF does not
create the bottleneck; it learns to aim through it.
...
The primary methodological limitation of this study is the sample size of the experimental
contrastive pairs. With $n=10$ per domain, each $\pm 0.10$ shift in transfer accuracy
represents a single sentence pair crossing the decision boundary; we therefore interpret
transfer accuracy as a directional indicator rather than a precise statistical measure.
Ongoing work is expanding the political and safety pair sets to $n \geq 100$ and will
supplement the binary transfer accuracy metric with continuous cosine similarity scores
and AUC-ROC curves to provide statistically robust evidence for the instillation effect.

Second, the four-model comparison---while consistent---does not yet constitute a
comprehensive survey. Concurrent experiments extending the direction trace to additional
Chinese-aligned families (DeepSeek-R1, Yi-1.5-9B, GLM-4-9B) and Western-aligned
families (OLMo-2, Phi-3.5, Gemma-2, Llama-3.1) are underway, as is a scaling
investigation using Qwen2.5-72B-Instruct to assess whether suppression strength
continues to grow with model capacity. These extended results will be reported
...
is consistent across tested architectures; the targeting is RLHF-specific.
