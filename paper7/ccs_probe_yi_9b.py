#!/usr/bin/env python3
"""
CCS Direction Trace — Yi-1.5-9B-Chat
Archon, DuoNeural, 2026-05-11

KEY QUESTION: Is suppression a Qwen-specific property or Chinese-RLHF universal?

Chinese-aligned suppressors found so far:
  Qwen2.5-14B-Instruct: 3.35× STRONG SUPPRESSOR
  Qwen2.5-7B-Instruct:  1.28× WEAK SUPPRESSOR
  DeepSeek-R1-7B:       7.46× SUPPRESSOR (reasoning amplifies it)

Yi-1.5-9B is from 01.AI — a different Chinese lab, different training pipeline.
If Yi is also a suppressor → Chinese RLHF universally suppresses political truth regardless of lab.
If Yi is neutral or crystallizer → Qwen's suppression is lab/training-recipe specific.

Architecture: Yi-1.5-9B has 48 layers, hidden_dim=4096
GPU: RTX 3060 12GB — 4-bit NF4 (~5GB VRAM)
Output: /workspace/yi_trace/direction_trace.json
"""
...
OUT_DIR  = Path("/workspace/yi_trace")
...
print(f"[yi15] {N_CTRL} ctrl + {N_EXP} exp = {len(ALL_PAIRS)} pairs", flush=True)

print(f"\n[yi15] Loading {MODEL_ID} 4-bit NF4...", flush=True)
...
print(f"[yi15] Loaded: {n_layers} layers, hidden={d}  VRAM: {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)
...
for i, (t, f) in enumerate(ALL_PAIRS):
    if i % 5 == 0: print(f"[yi15] pair {i+1}/{len(ALL_PAIRS)}", flush=True)
...
print(f"\n[yi15] Activations saved: {act_true.shape}", flush=True)
...
print("\n[yi15] Running direction trace...", flush=True)
trace = compute_trace(act_true, act_false, N_CTRL, N_EXP)

print(f"\n{'='*65}")
print(f"Direction Trace — Yi-1.5-9B-Chat ({n_layers} layers)")
print(f"{'='*65}")
print(f"{'L':>4}  {'|r|':>8}  {'angle':>7}  {'ctrl':>6}  {'exp':>6}  {'exp_gap':>10}")
for r in trace:
    flag = " << SUPP?" if r["angle_deg"] > 30 and r["layer"] > n_layers-5 else ""
...
print(f"\nComparison — Chinese-aligned models:")
print(f"  Qwen2.5-14B-Instruct:      3.35×  STRONG SUPPRESSOR")
print(f"  DeepSeek-R1-7B (reasoning): 7.46× SUPPRESSOR")
print(f"  Qwen2.5-7B-Instruct:        1.28×  WEAK SUPPRESSOR")
print(f"  Yi-1.5-9B-Chat (01.AI):    {ratio:.2f}×  {verdict}")
if ratio > 1.5:
    print("  ✓ Cross-lab suppression CONFIRMED — Chinese RLHF universally suppresses political truth")
else:
    print("  ? Suppression may be Qwen/DeepSeek-specific, not universal Chinese RLHF property")
...
out = {
    ...
    "comparison": {"qwen14b": 3.35, "deepseek_r1_7b": 7.46, "qwen7b": 1.28, "yi_9b": ratio}
}
with open(OUT_DIR / "direction_trace.json", "w") as f:
    json.dump(out, f, indent=2)
print(f"\n[yi15] Results saved → {OUT_DIR}/direction_trace.json")
