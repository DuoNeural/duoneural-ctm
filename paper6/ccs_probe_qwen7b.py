#!/usr/bin/env python3
"""
CCS Direction Trace — Qwen2.5-7B-Instruct
Archon, DuoNeural, 2026-05-11

Purpose: Test if the L_final suppression pattern found in Qwen2.5-14B
is SIZE-INVARIANT within the Chinese-aligned RLHF family.

Hypothesis: Qwen2.5-7B-Instruct will show the same final-layer
compression of the truth direction as Qwen2.5-14B.
Smaller model = less capacity, but RLHF objective same.

Qwen2.5-7B has 28 transformer layers, hidden_dim=3584.
Final transformer layer: L27 (0-indexed), trace index 28.
"""
...
OUT_DIR  = Path("/workspace/qwen7b_trace")
...
print(f"[qwen7b] {N_CTRL} ctrl + {N_EXP} exp = {len(ALL_PAIRS)} pairs", flush=True)

print(f"\n[qwen7b] Loading {MODEL_ID} 4-bit NF4...", flush=True)
...
print(f"[qwen7b] Loaded: {n_layers} layers, hidden={d}  VRAM: {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)
...
for i, (t, f) in enumerate(ALL_PAIRS):
    if i % 5 == 0: print(f"[qwen7b] pair {i+1}/{len(ALL_PAIRS)}", flush=True)
...
print(f"\n[qwen7b] Activations saved: {act_true.shape}", flush=True)
...
print("\n[qwen7b] Running direction trace...", flush=True)
trace = compute_trace(act_true, act_false, N_CTRL, N_EXP)

print(f"\n{'='*65}")
print(f"Direction Trace — Qwen2.5-7B-Instruct ({n_layers} layers)")
print(f"{'='*65}")
...
print(f"\nFinal-layer effect: L{n_layers-1}→L{n_layers}  "
      f"|r| {prev['magnitude']:.1f} → {final['magnitude']:.1f}  ({ratio:.2f}×)")
print(f"Rotation at final layer: {final['angle_deg']:.1f}°")
print(f"exp_acc: {prev['exp_acc']:.2f} → {final['exp_acc']:.2f}")
print(f"VERDICT: {verdict}")
print(f"\nQwen2.5-14B comparison: final-layer ratio was 3.35× (SUPPRESSOR)")
print(f"Qwen2.5-7B  measured:   final-layer ratio = {ratio:.2f}×")
if ratio > 1.5:
    print("✓ Size-invariant suppression CONFIRMED in Chinese-aligned family")
else:
    print("? Size-dependent — suppression may require model capacity")
...
print(f"\n[qwen7b] Done → {OUT_DIR}/direction_trace.json")
